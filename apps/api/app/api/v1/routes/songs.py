...
from app.schemas.schemas import AnalyzeLyricsResponse, SongListResponse, SongSummary
...
def get_saved_analysis(song_id: int, db: Session = Depends(get_db)):
    ...
    entries = entries_from_vocab_rows(rows)
    return build_analyze_response(song_id, lyric.clean_text, entries)
