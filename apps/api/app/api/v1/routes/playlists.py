...
def list_playlists(userId: int = Query(..., ge=1), db: Session = Depends(get_db)):
    ...
    for p in playlists:
        song_count = (
            db.query(func.count(PlaylistSong.id)).filter(PlaylistSong.playlist_id == p.id).scalar() or 0
        )
        summaries.append(
            PlaylistSummary(id=p.id, name=p.name, description=p.description, songCount=int(song_count))
        )
    return PlaylistListResponse(playlists=summaries)
