    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="card">
          ...
          {songs.length === 0 ? (
            <p className="text-sm text-textSecondary">
              No songs in the database for this user yet — or the API could not load them. Import a song, then refresh this page.
            </p>
          ) : (
        ...
        <div className="card space-y-3">
          ...
          {playlists.length === 0 ? (
            <p className="text-sm text-textSecondary">No playlists yet. Create one above, then add songs from a song&apos;s analysis page.</p>
          ) : (
            <ul className="space-y-2">
              {playlists.map((p) => (
                <li key={p.id}>
